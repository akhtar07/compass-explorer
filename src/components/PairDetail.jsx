      ...
      {d && (
        <div className="card glow p-4 fadein">
          ...
          {d.compounds?.length > 0 && (
            <>
              <div className="text-[11px] text-[var(--dim)] mb-1">DFT-stable compounds (convex hull) — formation energy eV/atom · spacegroup · source</div>
              <div className="flex flex-wrap gap-1.5">
                {d.compounds.map((c, i) => (
                  <span key={i} className="text-[11px] px-2 py-1 rounded-lg bg-[var(--panel2)] border border-[var(--border)]">
                    <b>{c.formula}</b> <span className="text-[var(--dim)]">{c.Ef}</span>
                    {c.spacegroup && <span className="text-[var(--dim)]"> · {c.spacegroup}</span>}
